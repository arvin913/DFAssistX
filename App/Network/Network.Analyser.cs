        ...
        private void HandleMessage(byte[] message)
        {
            try
            {
                ...
                /*if (opcode == 0x0002)
                {
                    ...
                    {
                        this.mainForm.overlayForm.Invoke(() => this.mainForm.overlayForm.Visible = false);
                    }
                }
                else if (opcode == 0x00C4)
                {
                    this.mainForm.overlayForm.CancelDutyFinder();
                    if (Settings.ShowOverlay && Settings.AutoOverlayHide)
                    {
                        this.mainForm.overlayForm.Invoke(() => this.mainForm.overlayForm.Visible = true);
                    }
                }*/
                ...
                else if (opcode == 0x0143)
                {
                    ...
                    else if (type == 0x74)
                    {
                        ...
                        if (Settings.FATEs.Contains(code))
                        {
                            ...
                            if (Settings.TTS)
                            {
                                voice.SpeakAsync(Data.getFateNickName(fate.Name) + "刷了");
                            }
                            if(Settings.Remind)
                            {
                                new SoundPlayer(App.Properties.Resources.remind).Play();
                            }
                        }
                    }
                }
                ...
                else if (opcode == 0x0078)
                {
                    ...
                    else if (status == 3)
                    {
                        ...
                        Settings.EurekaMode = false;
                        mainForm.overlayForm.CancelDutyFinder();
                        if (Settings.ShowOverlay && Settings.AutoOverlayHide &&!Settings.EurekaMode)
                        {
                            this.mainForm.overlayForm.Invoke(() => this.mainForm.overlayForm.Visible = true);
                        }
                    ...
                    else if (status == 4) //글섭에서 매칭 잡혔을 때 출력
                    {
                        ...
                        Settings.EurekaMode = false;
                        if (code==283 || code == 581|| code == 598 || code == 639)//优雷卡ID
                        {
                            Settings.EurekaMode = true;
                        }
